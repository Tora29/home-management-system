...
=== ドキュメントの目的
本設計書は、在庫管理システムにおけるFeatures層のクエリ関数（`getMasterData`）の設計と仕様を文書化したものです。
複数のエンティティにまたがるマスターデータの一括取得を担当する重要な関数の実装詳細を提供します。
...
=== 必要性
在庫管理システムにおいて、フォームの選択肢やマスターデータは頻繁に参照される重要な情報です。
これらのデータを効率的に取得することで、以下を実現します：
...
==== getMasterData関数
複数のマスターデータを並列で取得し、単一のオブジェクトとして返却する関数。

==== Promise.all並列処理
3つのデータベースクエリを同時実行し、全ての完了を待機。

==== フィルタリング条件
`isActive: true` によるアクティブデータのフィルタリング。

==== ソート処理
`sortOrder: 'asc'` による昇順ソート。
